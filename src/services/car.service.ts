...
//         // if (!(await hasPermission(user._id.toString(), "CREATE_CAR"))) {
//         //     throw new Error("У вас немає права створювати авто");
//         // }
//         //
//         // if (user.accountType === AccountType.BASE) {
//         //     const count = await Car.countDocuments({sellerId: user._id});
//         //     if (count >= 1) throw new Error("Basic акаунт може створити лише одне авто");
//         // }
//
...
//         // if (!car) throw new Error("Авто не знайдено");
//         // if (!car.sellerId.equals(user._id)) throw new Error("Не можна редагувати чуже авто");
//         //
//         // if (!(await hasPermission(user._id.toString(), "EDIT_CAR"))) {
//         //     throw new Error("У вас немає права редагувати авто");
//         // }
//
//
//         if (car.editAttempts >= 3) {
//             car.adStatus = AdStatusEnum.INACTIVE;
//             await emailService.sendToManager(car, {name: user.name, email: user.email});
//             return car.save();
//         }
//
...
export class CarService {
    ...
    public async getAllCars(query: GetCarsQuery, user?: IUser): Promise<ICar[]> {
        // Використовуємо Record<string, unknown> замість {}
