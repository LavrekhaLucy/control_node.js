...
export class CarMiddleware {
    ...
    //             role.permissions.includes('create_car') || role.permissions.includes('all')
    //         );
    //
    //         if (!allowed) return res.status(403).json({ message: 'You do not have permission to create a car' });
    //
    //         next();
    //     } catch (err) {
    //         next(err);
    ...
    public async checkProfanity(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            if (hasProfanity) {
                // ставимо прапорець і потрібний статус
                req.body.hasProfanity = true;
                req.body.profaneWords = words;
                req.body.adStatus = AdStatusEnum.PENDING;
            } else {
                // якщо немає матюків → можна активувати одразу
